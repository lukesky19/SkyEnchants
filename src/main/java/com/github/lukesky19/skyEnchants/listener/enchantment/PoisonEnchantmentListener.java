...
public class PoisonEnchantmentListener implements Listener {
    ...
    public void onEntityDamaged(EntityDamageByEntityEvent entityDamageByEntityEvent) {
        ...
        if(poison == null) {
            logger.error(AdventureUtil.deserialize("Unable to apply the poison enchantment's effects due to invalid settings."));
            return;
        }
        ...
        if(effectDurationPerLevel.isEmpty()) {
            logger.error(AdventureUtil.deserialize("Unable to apply the poison enchantment effect due to invalid plugin settings (No effect duration mapping)."));
            return;
        }

        // Log an error if the amplifier duration mapping is empty
        if(effectAmplifierPerLevel.isEmpty()) {
            logger.error(AdventureUtil.deserialize("Unable to apply the poison enchantment effect due to invalid plugin settings (No effect amplifier mapping)."));
            return;
        }
        ...
        for(EquipmentSlot equipmentSlot : equipmentSlots) {
            ...
            if(duration == null) {
                logger.error(AdventureUtil.deserialize("Unable to apply the poison enchantment effect due to invalid plugin settings (No effect duration mapping for enchantment level: " + enchantmentLevel + ")."));
                continue;
            }

            // Log an error if there is no amplifier mapping for the enchantment level and move to the next EquipmentSlot
            if(amplifier == null) {
                logger.error(AdventureUtil.deserialize("Unable to apply the poison enchantment effect due to invalid plugin settings (No effect amplifier mapping for enchantment level: " + enchantmentLevel + ")."));
                continue;
            }
