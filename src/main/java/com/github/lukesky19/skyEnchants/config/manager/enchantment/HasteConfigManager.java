...
/**
 * This class manages the {@link Haste} configuration.
 */
public class HasteConfigManager extends EnchantmentConfigManager<Haste> {
    /**
     * Constructor
     * @param dataFolder The plugin's data folder.
     * @param logger The plugin's {@link ComponentLogger}.
     */
    public HasteConfigManager(@NonNull File dataFolder, @NonNull ComponentLogger logger) {
        super(dataFolder, logger, "haste.yml", Haste.class);
    }

    /**
     * There is currently no migration required so the value is just returned.
     * @param haste The {@link Haste} configuration.
     * @return The original {@link Haste} configuration.
     */
    @Override
    public @Nullable Haste migrateConfiguration(@NonNull Haste haste) {
        return haste;
    }
